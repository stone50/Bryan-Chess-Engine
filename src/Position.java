...
public class Position {
	...
	if (whiteMove) {
		...
		if (currentMove.substring(0, 1).equals("O")) {

		} else if (currentMove.substring(1, 2).equals("-")) {
		    ...
		    int fiftyTemp;
		    if (test[srow][scol] == 'P' || test[frow][fcol] != '-') {
			fiftyTemp = 0;
		    } else {
			fiftyTemp = fiftyMoveRule + 1;
		    }
		    ...
		    test[srow][scol] = '-';
		    Position nextPos = new Position(test, !whiteMove, castle, ep, fiftyTemp, moveCount);
		    double nextEval;
		    if (Math.abs(SE - nextPos.SE()) < depthThreshold) {
			nextEval = nextPos.think(depthThreshold, Math.min(depth - 1, depth % 2));
		    } else {
		...
		if (currentMove.substring(0, 1).equals("O")) {
		    ...
		    test[srow][scol] = '-';
		    Position nextPos = new Position(test, !whiteMove, castle, ep, fiftyTemp, moveCount + 1);
		    double nextEval;
		    if (Math.abs(SE - nextPos.SE()) < depthThreshold) {
			nextEval = nextPos.think(depthThreshold, Math.min(depth - 1, depth % 2));
		    } else {
